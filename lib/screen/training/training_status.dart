...
import 'package:andapp/screen/training/training_status_bloc.dart';
...
class TrainingStatus extends StatefulWidget {
  final String title;
  const TrainingStatus({Key? key,required this.title}) : super(key: key);
  @override
  State<TrainingStatus> createState() => _TrainingStatusState();
}

class _TrainingStatusState extends State<TrainingStatus> with TickerProviderStateMixin{
  final TrainingStatusBloc bloc = TrainingStatusBloc();
  ...
  Widget build(BuildContext context) {
    ...
    final scrollController = ScrollController(
        initialScrollOffset: current * 50);
    return Scaffold(
      appBar: AppBar(
          title: Align(
                          ...
                          Container(
                            ...
                            child: ListView.separated(
                                ...
                                itemBuilder: (BuildContext context, int index) {
                                  ...
                                  for (int i = 0; i <
                                      listQuestion.length; i++) {
                                    if (listQuestion[i].attemptedAns !=
                                        null) {
                                  ...
                                  return Transform.scale(
                                    ...
                                    child: FloatingActionButton(
                                        ...
                                        backgroundColor: (index == current ||
                                            listQuestion[index].attemptedAns !=
                                                null)
                                            ? appTheme
                                            .primaryColor
                                            : Colors.transparent,
                                        onPressed: () {
                                          ...
                                          if (index <= current + 1 || index <=
                                              latestAttemptedIndex + 1) {
                                            selectIndex(
                                          ...
                                          children: <Widget>[
                                            SvgPicture.asset(
                                              listQuestion[current]
                                                  .attemptedAns ==
                                                  StringUtils.ansA ? SvgImages
                                                  .radioSelected : SvgImages
                                                  .radioUnSelected, height: 22,
                                              width: 22,),
                                    ...
                                    child: PinkBorderButton(
                                      ...
                                      onPressed: () async {
                                        if (nextButton ==
                                          ...
                                          await bloc
                                              .submitAnswers(
                                              context,widget.title, listQuestion);
                                        }
                                        else {
                                          if (current <
                                              listQuestion.length - 1) {
  ...
  void selectIndex(int index,ScrollController siScrollController) {
    ...
    _pageController.jumpToPage(
        index); // for regular jump
    _pageController.animateToPage(
        index, curve: Curves.decelerate,
        duration: const Duration(
            milliseconds: 300)); // for animated jump. Requires a curve and a duration
    if (index > 2) {
      siScrollController.animateTo(
          (current - 2) * 68,
          duration: const Duration(
              milliseconds: 300),
